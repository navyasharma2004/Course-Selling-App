...
export default function AdminAuth() {
  ...
  return (
    <div className="max-w-sm mx-auto px-6 py-16">
      <p className="font-sans text-xs tracking-wide text-gold mb-2">Admin</p>
      <h1 className="font-serif text-3xl text-ink mb-1">
        {mode === "signin" ? "Admin sign in" : "Create an admin account"}
      </h1>
      <p className="font-sans text-sm text-ink/60 mb-8">Manage the course catalogue.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-ink text-paper font-sans text-sm py-2.5 hover:bg-ink/80 transition-colors disabled:opacity-50"
        >
      ...
      >
        {mode === "signin" ? "New admin? Create an account" : "Already have an account? Sign in"}
      </button>
    </div>
  );
}
